<template>
  <div class="me" v-if="userInfo._id">
    <router-link tag="div" class="user" to="/detail">
      ...
      <p v-if="userInfo.tel">{{userInfo.tel|telFormat}}</p>
    ...
    <div class="setting">
      ...
      <div class="setting-item" @click.prevent="logout">
        <i class="itlike-4"></i>
        <span>退出</span>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...mapActions(['reqUserInfoDelete']),
      logout(){
        MessageBox.confirm('确定退出吗?').then((actions)=>{
          if(actions==='confirm'){
            this.reqUserInfoDelete()
